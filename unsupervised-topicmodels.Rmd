...
author: "Yinuo Liu,Xiangyuanxi Zhang..........."
date: "14/03/2024"
...
head(df_glove_umap)
target <- "climate"
cos_sim <- sim2(x = glove_embedding, y = glove_embedding[target, , drop = FALSE], method = "cosine")
nearest <- head(sort(cos_sim[,1], decreasing = TRUE), 15) 
print(nearest)


selected_words <- df_glove_umap %>% filter(word %in% names(nearest))

ggplot(selected_words, aes(x = UMAP1, y = UMAP2)) +
  geom_point() +
  geom_text(aes(label = word), vjust = -1, hjust = 0.5, size = 3) +
  labs(title = paste("Nearest neighbours of", target)) +
  theme_minimal()

#exercise2
region_words <- df_glove_umap %>%
  filter(UMAP1 > -2, UMAP1 < 0, UMAP2 > 4, UMAP2 < 6) %>%
  pull(word)
head(region_words, 20)   

region_df <- df_glove_umap %>% filter(word %in% region_words)
ggplot(region_df, aes(x = UMAP1, y = UMAP2)) +
  geom_point() +
  geom_text(aes(label = word), vjust = -1, hjust = 0.5, size = 2.5) +
  labs(title = "Words in a selected UMAP region") +
  theme_minimal()
```
